...
def TifCrop(TifPath, SavePath, CropSize, RepetitionRate):
    ...
    # 获取文件名
    raw_file_name, extension = os.path.splitext(os.path.basename(TifPath))

    width = dataset_img.RasterXSize  # 获取行列数？ RasterXSize为宽度，代表列；RasterYSize为高度，代表行
    ...
    for i in range(RowNum):
        for j in range(ColumnNum):
            ...
            # 生成Tif图像
            new_file_name = "/%s_%d_%d.tif" % (raw_file_name, row_index, column_index)
            print("new file name:",new_file_name)
            writeTiff(cropped, crop_geotrans, proj, SavePath + new_file_name)
...
if __name__ == '__main__':
    ...
    # # dom
    # TifCrop(r"F:\A-dataset-base\open-pit-mine-p10-raw\OPM_DOM.tif",
    #         r"F:\A-dataset-base\open-pit-mine-p10-slice\RGB1024", 1024, 0)

    # dsm
    # TifCrop(r"F:\A-dataset-base\open-pit-mine-p10-raw\OPM_DSM.tif",
    #         r"F:\A-dataset-base\open-pit-mine-p10-slice\DSM1024", 1024, 0)

    # dsm 图像宽：59393 图像高：97535预估大小：16.2 GB
    TifCrop(r"F:\A-dataset-base\open-pit-mine-p10-raw\OPM_DOM.tif",
            r"F:\A-dataset-base\open-pit-mine-p10-half", 59393, 0)